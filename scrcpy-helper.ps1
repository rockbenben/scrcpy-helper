...
$customAppsPath = Join-Path $PSScriptRoot '投屏助手-常用应用.json'
$script:customApps = [ordered]@{}   # 用户自定义的常用 App（名称 => 包名），持久化到上面的文件
...
# 用户自定义常用 App（独立窗口下拉里那几个之外、用户自己加的），单独存一个文件，避免被设置的类型转换吃掉
function Load-CustomApps {
    if (-not (Test-Path -LiteralPath $customAppsPath)) { return }
    try {
        $j = Get-Content -LiteralPath $customAppsPath -Raw -Encoding UTF8 | ConvertFrom-Json
        foreach ($p in $j.PSObject.Properties) {
            if ($p.Name -and $p.Value) { $script:customApps[[string]$p.Name] = [string]$p.Value }
        }
    } catch { }
}
function Save-CustomApps {
    try {
        $o = [ordered]@{}
        foreach ($k in $script:customApps.Keys) { $o[$k] = $script:customApps[$k] }
        ($o | ConvertTo-Json) | Set-Content -LiteralPath $customAppsPath -Encoding UTF8
    } catch { }
}
...
# 读取已连接手机的 Android 大版本号（整数，如 13）；没连设备/多设备/读不到都返回 0（=未知，不拦截）
function Get-AndroidVer {
    try {
        $devs = Get-DeviceList
        if (-not $devs -or $devs.Count -ne 1) { return 0 }
        $v = (& $adb -s $devs[0] shell getprop ro.build.version.release 2>$null | Select-Object -First 1)
...
# 设备「型号 + 安卓版本」展示串，按序列号缓存（同一台只读一次 adb，避免每次刷新都跑命令）
$script:devInfo = @{}
function Get-DevInfo {
    ...
    $txt = ''
    try {
        $ver   = ((& $adb -s $serial shell getprop ro.build.version.release 2>$null | Select-Object -First 1) | Out-String).Trim()
        $model = ((& $adb -s $serial shell getprop ro.product.model       2>$null | Select-Object -First 1) | Out-String).Trim()
        if ($ver)   { $txt = "Android $ver" }
        if ($model) { $txt = if ($txt) { "$txt · $model" } else { $model } }
    } catch {}
    $script:devInfo[$serial] = $txt
    return $txt
}
...
function Show-AppPicker {
    ...
    $dlg = New-Object System.Windows.Forms.Form
    $dlg.Text = '选择 App'; $dlg.ClientSize = New-Object System.Drawing.Size(340, 388)
    $dlg.Icon = Get-AppIcon
    $dlg.StartPosition = 'CenterParent'; $dlg.FormBorderStyle = 'FixedDialog'
    $dlg.MaximizeBox = $false; $dlg.MinimizeBox = $false; $dlg.Font = $owner.Font; $dlg.BackColor = $cPaper
    ...
    $fill = {
        ...
        foreach ($it in $list) {
            if (-not $q -or $it.Name -like "*$q*" -or $it.Pkg -like "*$q*") {
...
function Show-ManageApps {
    param($owner)
    $dlg = New-Object System.Windows.Forms.Form
    $dlg.Text = '我的常用应用'; $dlg.ClientSize = New-Object System.Drawing.Size(360, 322)
    $dlg.Icon = Get-AppIcon
    $dlg.StartPosition = 'CenterParent'; $dlg.FormBorderStyle = 'FixedDialog'
    $dlg.MaximizeBox = $false; $dlg.MinimizeBox = $false; $dlg.Font = $owner.Font; $dlg.BackColor = $cPaper
        ...
        if ($sel -like '更多应用*') {
            ...
            if (-not $apps.Contains($app.Name) -and -not $script:customApps.Contains($app.Name)) {
                if ([System.Windows.Forms.MessageBox]::Show("要把「$($app.Name)」加到常用列表吗？以后可直接在下拉里选它。", '常用应用', 'YesNo', 'Question') -eq 'Yes') {
                    $script:customApps[$app.Name] = $app.Pkg; Save-CustomApps
                }
            }
        }
...
function Show-WirelessPair {
    param($owner)
    $dlg = New-Object System.Windows.Forms.Form
    $dlg.Text = '用配对码连接（免插线）'; $dlg.ClientSize = New-Object System.Drawing.Size(360, 262)
    $dlg.Icon = Get-AppIcon
    $dlg.StartPosition = 'CenterParent'; $dlg.FormBorderStyle = 'FixedDialog'
    $dlg.MaximizeBox = $false; $dlg.MinimizeBox = $false; $dlg.Font = $owner.Font; $dlg.BackColor = $cPaper
...
try {
    ...
    $updateStatus = {
        ...
        if ($devs) {
            ...
            $lblDevInfo.Text = Get-DevInfo $dev
        } else {
            $lblStatus.BackColor = $cTagBg; $lblStatus.ForeColor = $cMuted
            $lblStatus.Text = '○ 未连接'
            $lblDevInfo.Text = ''
    ...
    $btnWireless.Add_Click({
        ...
        elseif ($usb) {
            ...
            $pick.Text = '无线投屏'; $pick.ClientSize = New-Object System.Drawing.Size(360, 226)
            $pick.Icon = Get-AppIcon
            $pick.StartPosition = 'CenterParent'; $pick.FormBorderStyle = 'FixedDialog'
            $pick.MaximizeBox = $false; $pick.MinimizeBox = $false; $pick.Font = $form.Font; $pick.BackColor = $cPaper

            $pl = New-Lbl '没有检测到手机，选一种无线连接方式：' 20 18
            $btnCable = New-PrimaryBtn '插数据线连接（推荐 · 最省事）' 20 50 320 48 11
            $capCable = New-Lbl '插一次线即可，连上后自动切无线、可拔线。' 24 100; $capCable.ForeColor = $cMuted; $capCable.Font = New-Object System.Drawing.Font('Microsoft YaHei UI', 8.5)
            $btnPair = New-SecondaryBtn '用配对码连接（Android 11+ · 免插线）' 20 128 320 44
            $capPair = New-Lbl '手机开「无线调试」，与电脑连同一个 Wi-Fi。' 24 174; $capPair.ForeColor = $cMuted; $capPair.Font = New-Object System.Drawing.Font('Microsoft YaHei UI', 8.5)
