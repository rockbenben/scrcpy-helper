...
- 🗂️ **多设备管理 + 同时多投**：「设备管理」里连接与投屏分开，可重命名 / 设默认 / 断开 / 忘记；连过的无线设备自动记住。支持「全部投屏」/ 多选投屏，多台各开一个窗口同时镜像（窗口标题即设备名、列表标「▶ 投屏中」），「停止投屏」可单独关掉某一台。
- 📷 **摄像头按机型自适配**：分辨率按摄像头实际支持的尺寸自动选（`--camera-ar` + 最大边长），根治写死 1920x1080 在部分机型/前置摄像头上的 `Camera configuration error`；档位可选高 / 中 / 原始最高。
